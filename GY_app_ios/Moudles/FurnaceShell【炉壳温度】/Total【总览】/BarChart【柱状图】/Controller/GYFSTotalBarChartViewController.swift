//
//  GYFSTotalBarChartViewController.swift
//  GY_app_ios
//
//  Created by zhaoenjia on 2023/11/9.
//
...
extension GYFSTotalBarChartViewController {
    func setupViews() {
        self.title = "柱状图"
...
extension GYFSTotalBarChartViewController {
    ...
    func requestnextdata(array:NSArray){
        ...
        GYNetworkManager.share.requestData(.get, api: Api.getlkzonglanzhuzhuangtu, parameters: params) {[weak self] (result) in
            guard let weakSelf = self else{
                return
            }
...
//        if array.count > 10 {
//            for temp in array.prefix(10) {
//                guard let tempp = temp as? NSDictionary else {
//                    return
//                }
//                let temppmodel = GYFSDataModel.deserialize(from: tempp)!
//                data.add(temppmodel.tempTagValue)
//                categories.add(temppmodel.name)
//            }
//
//            // 将NSArray转换为Swift中的数组类型
//            let swiftArray = array as! [NSArray]
//            // 按照十个一组重新分割数组
//            let result = chunkArray(array: swiftArray as! [NSDictionary], chunkSize: 10)
//            //绘制索引页
//            drawindexview(array: result)
//        }else{
//            for temp in array {
//                guard let tempp = temp as? NSDictionary else {
//                    return
...
extension GYFSTotalBarChartViewController:UIPickerViewDelegate,UIPickerViewDataSource,AAChartViewDelegate {
...
extension GYFSTotalBarChartViewController:LMJDropdownMenuDelegate,LMJDropdownMenuDataSource{

...
class GYFSTotalCell: UICollectionViewCell {
    static let indentifier: String = "GYFSTotalCell"
...
extension GYFSTotalCell:UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.titleStr = "温度"
        cell?.valueStr = (dataModel?.tempTagValue)!
        if dataModel?.isException == 1 {
            cell?.isredColor = true
        }else if dataModel?.isException == 3 {
            cell?.isyellowColor = true
        }else if dataModel?.isException != 0 {
            cell?.isredColor = true
        }else{

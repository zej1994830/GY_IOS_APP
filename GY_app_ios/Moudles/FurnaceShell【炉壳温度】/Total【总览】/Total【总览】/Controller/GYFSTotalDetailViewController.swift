//
//  GYFSTotalDetailViewController.swift
//  GY_app_ios
//
...
class GYFSTotalDetailViewController: GYViewController {
    ...
    var model:GYFSDataModel = GYFSDataModel() {
        didSet{
            titleLable.text = String(format: "【%@】", model.partName!)
            
            dataArray = [["温度(°C)",model.tempTagValue],["温度(°C)",model.tempTagValue],["温度(°C)",model.tempTagValue]]
            dataArray2 = [["温度",String(format: "主机地址：%d", model.masterAddress!),String(format: "子机地址：%d", model.slaveAddress!)]]
        }
    }
...
extension GYFSTotalDetailViewController {
    func setupViews() {
    ...
    @objc func trendBtnClick() {
        let vc = GYFSTotalTrendViewController()
        vc.model = model
        self.navigationController?.pushViewController(vc, animated: true)
    }
}

extension GYFSTotalDetailViewController:UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout {

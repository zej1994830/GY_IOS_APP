    ...
    var dataStrArray:NSMutableArray = [] {
        didSet{
            ...
            if dataStrArray.count == 0 {
                dataArray.addObjects(from: [["温差",dataModel?.wcValue],["热流",dataModel?.reFlowTagValue],["入温",dataModel?.inTagValue],["出温",dataModel?.outTagValue],["流量",dataModel?.flowTagValue]])
            }else{
                for temp in dataStrArray {
                    ...
                    if tempStr == "流量"{
                        dataArray.addObjects(from:[["流量",dataModel?.flowTagValue]])
                    }
                }
            }
